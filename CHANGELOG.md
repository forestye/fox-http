...
- `dispatch()` 里 `handler_.handle(*request, response)` 外围 try/catch
  （捕获 `std::exception` 和 `...`）。如果响应头还没发送，调用新增
  `replace_with_500()` 清空 response 并写出 500 + 异常消息；如果响应已经
  在发送中（状态行和 headers 已写入 socket），log 后关连接。
- `post_stream_work()` 里 stream lambda 调用 + `end_chunks()` 整体外围 try/catch。
  响应头还没发送时在 handler 线程上同步发 500 响应，然后 finish。
