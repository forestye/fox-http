/*
实现了HttpRequest类的方法。
parse()方法接受一个输入流，从中读取并解析请求行和头字段。
parse_request_line()和parse_headers()是辅助方法，分别负责解析请求行和头字段。

get_method()、get_path()、get_version()和get_header()方法用于访问请求的属性。
*/
...
bool HttpRequest::is_valid_header(const std::string& name) {
    // This is a basic implementation; you can extend this to validate header names more thoroughly
